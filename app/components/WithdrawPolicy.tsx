...
function SectionBody({ section }: { section: WithdrawalSection }) {
  const paragraphs = section.paragraphs ?? [];
  const items = section.items ?? [];
  const formLines = section.formLines ?? [];

  if (section.id === "defective-incorrect-products") {
    return (
      <div className="space-y-4">
        {paragraphs[0] ? <p>{paragraphs[0]}</p> : null}
        {paragraphs[1] ? (
          <p>{renderTextWithEmail(paragraphs[1])}</p>
        ) : null}
  ...
  if (section.id === "model-withdrawal-form") {
    return (
      <div className="space-y-4">
        {paragraphs.map((paragraph) => (
          <p key={paragraph}>{paragraph}</p>
        ))}
        <div className="rounded-xl border border-beige bg-beige/20 p-5 sm:p-6">
          {section.formTitle ? (
            <p className="font-semibold text-dark-green">{section.formTitle}</p>
          ) : null}
          <div className="mt-4 space-y-3 text-sm sm:text-base">
            {formLines.map((line) => (
              <p key={line}>{renderTextWithEmail(line)}</p>
            ))}
          </div>
        </div>
      </div>
