...
function SectionBody({ section }: { section: ReturnsSection }) {
  const paragraphs = section.paragraphs ?? [];
  const items = section.items ?? [];

  if (section.id === "defective-damaged-incorrect") {
    return (
      <div className="space-y-4">
        {paragraphs[0] ? (
          <p>{renderTextWithEmail(paragraphs[0])}</p>
        ) : null}
...
export default function ReturnsRefundDetails() {
  const { quickNavigation, sections } = useMessages().returnsRefunds;
